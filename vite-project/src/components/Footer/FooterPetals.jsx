 ...
 *
 * ── Transform ownership split ───────────────────────────────────
 * Each rendered petal is two elements:
 *
 *   <div class="footer-petal-wrap">   ← GSAP owns this: x, y, rotation, opacity
 *     <img class="footer-petal-img">  ← CSS inline owns: static rotate + scale
 *   </div>
 */
...
const FooterPetals = () => {
  ...
  useEffect(() => {
    // Seed GSAP's transform cache on every wrapper so it fully owns
    // x / y / rotation / opacity — nothing conflicts with CSS transforms.
  ...
  const animatePetal = (index) => {
    ...
    // Kill any in-progress animation on this petal
    gsap.killTweensOf(wrap);

    const leftPct = parseFloat(p.left);
    // side: -1 = far left, 0 = centre, +1 = far right
    const side = (leftPct - 50) / 50;

    // Large, clear outward trajectory so the flight is unmistakable
    const flyX     = side * (150 + Math.random() * 100) + (Math.random() - 0.5) * 50;
    const flyY     = -(180 + Math.random() * 120);
    const rotDelta = (side >= 0 ? 1 : -1) * (80 + Math.random() * 80);

    const tl = gsap.timeline();

    // ── Phase 1: fly away (move + fade together so motion is clear) ──
    tl.to(wrap, {
      x:        flyX,
      y:        flyY,
      rotation: rotDelta,
